...
describe('useSearch', () => {
  ...
  it('clear 應該清除所有狀態', async () => {
    const { search, clear, results, keyword, error, updateKeyword } = useSearch({ autoSearch: false })
  ...
  it('應該正確設定 matchType', async () => {
    ...
    const exactMatch = results.value.find((r: { matchType: string }) => r.matchType === 'TITLE_EXACT')
