...
================================================================================
版本變更: 1.6.1 → 1.6.2 (補充函式命名註解原則與實際案例)
修改的原則:
  - II. 程式碼品質 → 程式碼註解規範：補充「函式命名已明確表達意圖時不需註解」原則，並新增實際移除冗餘註解的案例說明
新增區段: 
  - 新增「註解審查實踐案例」小節，記錄 2025-12-21 移除冗餘註解的具體操作
移除區段: 無
需要更新的範本:
  - `.specify/templates/plan-template.md` ✅ 已審查（無需更新）
  - `.specify/templates/spec-template.md` ✅ 已審查（無需更新）
  - `.specify/templates/tasks-template.md` ✅ 已審查（無需更新）
  - `.specify/templates/checklist-template.md` ✅ 已審查（無需更新）
  - `.specify/templates/agent-file-template.md` ✅ 已審查（無需更新）
待辦事項: 無
上次修改: 1.6.0 → 1.6.1 (精煉註解規範：明確 template 註解原則)
================================================================================
-->
...
- 需求實作 MUST 讓所有相關測試從失敗狀態（紅燈 🔴）變為通過狀態（綠燈 🟢）
- E2E 測試 MUST 通過：執行 `pnpm run test:e2e` 或 Playwright 測試後顯示全部通過
- 單元測試 MUST 通過：執行 `pnpm run test` 或 Vitest 測試後顯示全部通過
- 只有當 E2E 測試和單元測試都從紅燈變成綠燈，需求才算真正完成
- 測試通過 MUST 在本地環境和 CI/CD 環境中一致

**理由**：測試優先確保程式碼的可測試性設計，減少回歸錯誤，並作為功能的活文件。明確的完成定義確保品質標準，避免未經驗證的程式碼進入主分支。
...
- 以下情況 MUST NOT 撰寫註解（視為冗餘註解）：
  ...
  - 函式內部與函式名稱重複的註解（如 `// 執行搜尋` 配 `function performSearch()`）：函式名稱已明確表達意圖
  ...
  - 與 template 語意明確相符的註解（如 `<!-- 錯誤訊息 -->` 配 `<div v-if="error">{{ error }}</div>`）：template 結構本身已清楚表達功能
- 以下情況 SHOULD 撰寫註解：
  - 複雜的業務邏輯或演算法
  - 非直觀的技術決策及其理由
...
- Template 註解原則：
  ...
  - 範例：`<div v-if="error" class="...">{{ error }}</div>` 已明確表達「當有錯誤時顯示錯誤訊息」，不需額外註解 `<!-- 錯誤訊息 -->`
  - 範例：`<SearchBar />` 元件名稱已表達功能，不需註解 `<!-- 搜尋框 -->`
  - 僅在 template 邏輯複雜、非直觀或需要特別說明時才添加註解
- **註解審查實踐案例（2025-12-21）**：
  - 移除 `SearchPage.vue` 第 77 行 `// 執行搜尋` 註解：函式名稱 `performSearch()` 已明確表達搜尋執行的意圖
  - 原則：當函式命名已清楚表達其行為時，函式內部不需要重複說明該行為的註解
  - 判斷標準：閱讀函式名稱時，如果註解內容與函式名稱表達的意思完全相同，則該註解為冗餘
...
**Version**: 1.6.1 | **Ratified**: 2025-11-26 | **Last Amended**: 2025-12-20
