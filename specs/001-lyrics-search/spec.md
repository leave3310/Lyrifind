...
- 當 Google Sheets API 請求失敗時，系統應顯示錯誤訊息並提供重試按鈕
...
- **FR-014**: 系統 MUST 支援點擊搜尋按鈕或按下 Enter 鍵觸發搜尋
...
- **FR-016**: 搜尋結果 MUST 使用傳統分頁導覽，每頁顯示 20 筆結果
...
- **FR-015**: 當 API 請求失敗時，系統 MUST 顯示錯誤訊息並提供重試按鈕

### Key Entities

- **Song（歌曲）**: 代表一首歌曲，對應 Google Sheets 欄位：
  - `id`：唯一識別碼
  - `title`：歌曲名稱
  - `artist`：歌手名稱
  - `lyrics`：完整歌詞內容
...
- Q: 歌詞資料來源策略？ → A: 使用自建資料庫，資料儲存於 Google Sheets
- Q: Google Sheets 資料欄位結構？ → A: 標準版四欄（ID、歌名、歌手、歌詞）
- Q: 搜尋觸發方式？ → A: 按鈕點擊 + Enter 鍵觸發搜尋
- Q: API 失敗時的處理策略？ → A: 顯示錯誤訊息 + 提供重試按鈕
- Q: 搜尋結果分頁方式？ → A: 傳統分頁導覽（頁碼導覽列）

## Assumptions

- 歌詞資料來源將使用自建資料庫，透過 Google Sheets 儲存和管理歌詞資料
- 透過 Google Sheets API 讀取歌詞資料
- 初期版本不需要使用者帳號系統
- 搜尋結果分頁預設每頁顯示 20 筆
- 支援中文和英文歌詞搜尋
