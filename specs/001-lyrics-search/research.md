...
### 決策：使用原生 JavaScript 實作 debounce + Composable 模式

### 理由

- 避免額外的相依套件（VueUse 功能強大但套件較大）
- 符合專案憲章「優先使用原生 Web API 和 Vue 內建功能」原則
- debounce 實作簡單，不需要整個函式庫
...
| L2 快取 | LocalStorage | 完整歌詞資料 | 1 小時 |
...
| 方案 | 優點 | 缺點 | 決策 |
|------|------|------|------|
| VueUse `useDebounceFn` | 穩定可靠、功能豐富 | 需額外安裝套件 | ❌ 拒絕 |
| Lodash debounce | 穩定可靠 | 需額外安裝、無 Vue 整合 | ❌ 拒絕 |
| 自行實作 debounce | 無相依性、符合憲章 | 維護成本低（功能簡單） | ✅ 採用 |
...
| 搜尋防抖 | 原生 JavaScript debounce | 🔴 高 |
| 快取策略 | Map + LocalStorage 雙層快取 | 🟡 中 |
